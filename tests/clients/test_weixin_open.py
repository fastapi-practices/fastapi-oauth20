...
from fastapi_oauth20.errors import GetUserInfoError, HTTPXOAuth20Error, RefreshTokenError
...
class TestWeChatOpenOAuth20:
    def test_client_initialization(self, wechat_open_client):
    ...
    def test_client_inheritance(self, wechat_open_client):
        assert isinstance(wechat_open_client, OAuth20Base)

    def test_client_scopes_are_lists(self, wechat_open_client):
    ...
    def test_client_endpoint_urls(self, wechat_open_client):
        assert wechat_open_client.authorize_endpoint.endswith('/connect/qrconnect')
        assert wechat_open_client.access_token_endpoint.endswith('/sns/oauth2/access_token')
        assert wechat_open_client.refresh_token_endpoint.endswith('/sns/oauth2/refresh_token')
        assert wechat_open_client.userinfo_endpoint.endswith('/sns/userinfo')
        assert 'open.weixin.qq.com' in wechat_open_client.authorize_endpoint
        assert 'api.weixin.qq.com' in wechat_open_client.access_token_endpoint
    ...
    async def test_get_authorization_url_with_custom_scope(self, wechat_open_client):
        redirect_uri = 'https://example.com/callback'
        url = await wechat_open_client.get_authorization_url(redirect_uri=redirect_uri, scope=['snsapi_login'])
        assert 'scope=snsapi_login' in url

    @pytest.mark.asyncio
    async def test_get_authorization_url_with_lang_parameter(self, wechat_open_client):
        url = await wechat_open_client.get_authorization_url(
    ...
    @pytest.mark.asyncio
    async def test_get_authorization_url_with_kwargs(self, wechat_open_client):
        url = await wechat_open_client.get_authorization_url(
    ...
    async def test_get_userinfo_success(self, wechat_open_client):
        ...
        result = await wechat_open_client.get_userinfo(TEST_ACCESS_TOKEN, openid='test_openid')
    ...
    async def test_get_userinfo_with_lang_parameter(self, wechat_open_client):
        ...
        await wechat_open_client.get_userinfo(TEST_ACCESS_TOKEN, openid='test_openid')
        assert route.called
    ...
    async def test_get_userinfo_http_error_401(self, wechat_open_client):
        ...
        with pytest.raises(HTTPXOAuth20Error):
            await wechat_open_client.get_userinfo(INVALID_TOKEN, openid='test_openid')
